...
class ProjectConfig(BaseSettings):
    ...
    APP_TITLE: str = "Toolbox - 测管平台"
    # Swagger/ReDoc 头部描述：CommonMark语法渲染，每行必须顶格（缩进会被解析为代码块）
    APP_DESCRIPTION: str = """
**ToolBox - 测管平台**：
一款基于 Python FastAPI 的实用型测试管理系统，覆盖测试用例管理、环境配置、任务调度与报告生成的完整闭环。
...
- **HTTP请求 / TCP请求**：发送 HTTP/HTTPS 或 TCP 协议请求，支持变量提取、断言与参数化数据驱动
- **数据库请求 / Redis请求**：按环境配置连接目标服务执行 SQL / Redis 命令，结果可存为会话变量
- **代码请求(Python)**：执行自定义 Python 代码，编排复杂业务逻辑
