...
class AutoTestEnvBase(BaseModel):
    ...
    env_desc: Optional[str] = Field(None, max_length=2048, description="绑定描述")
...
class AutoTestEnvCreate(AutoTestEnvBase):
    ...
    env_type: AutoTestConfigNodeType = Field(..., description="节点类型")
...
class AutoTestEnvDelete(BaseModel):
    ...
    env_ids: Optional[List[int]] = Field(None, description="环境绑定ID列表")
...
class AutoTestEnvListQuery(BaseModel):
    ...
    project_id: Optional[List[int]] = Field(None, description="应用ID列表")
...
class AutoTestEnvPageSelect(BaseModel):
    ...
    env_type: Optional[AutoTestConfigNodeType] = Field(None, description="节点类型")
    ip: Optional[str] = Field(None, max_length=128, description="IP地址")
