...
class AutoTestCaseModel(ScaffoldModel, MaintainMixin, TimestampMixin, StateModel, ReserveFields):
    ...
    # case_tags 存储为List[int]格式, 每个元素都是Tag记录的主键
    ...
    case_steps = fields.IntField(default=0, ge=0, description="用例步骤数量")
    case_state = fields.BooleanField(null=True, description="用例执行状态")
    case_version = fields.IntField(default=1, ge=1, description="用例更新版本")
    case_project = fields.IntField(default=1, ge=1, index=True, description="用例所属应用")
    case_last_time = fields.DatetimeField(null=True, description="用例最后一次执行时间")
    ...
    # owner_user 创建时等于创建人员created_user, 修改时不记录、用例转让时记录实际操作人员
