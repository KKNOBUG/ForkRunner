...
class AutoTestReportModel(ScaffoldModel, MaintainMixin, TimestampMixin, StateModel, ReserveFields):
    ...
    case_state = fields.BooleanField(null=True, description="用例执行状态")

    step_total = fields.IntField(default=0, ge=0, description="用例步骤数量")
    step_fail_count = fields.IntField(default=0, ge=0, description="用例步骤失败数量")
    step_pass_count = fields.IntField(default=0, ge=0, description="用例步骤成功数量")
    step_pass_ratio = fields.FloatField(default=0.0, ge=0.0, description="用例步骤成功率")

    batch_code = fields.CharField(max_length=64, default=None, null=True, index=True, description="批次标识代码")
